...
function exportDataExtended(datatype, dataObject) {
	console.log();
		...
		for(var idUserSample in dataObject[gestureName])
		{
		    ...
		    for(let i = 0; i < gesture.length; i++) {
			    csvContentUserSample += gesture[i].timestamp + DELIM + 
			    						...
			    						gesture[i].emg[7] + DELIM + 
			    						gesture[i].direction[0] + DELIM + 
			    						gesture[i].direction[1] + DELIM + 
			    						gesture[i].direction[2] + "\n";
		    }
...
module.exports = exportDataExtended;